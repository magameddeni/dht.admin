...
import { usePhoneMask } from "hooks/index"
...
const Input = React.forwardRef(
  (
    ...
    useEffect(() => {
      if (!inputActive && !Array.isArray(value) && !!value)
    ...
    return (
      <label htmlFor={id} className={formInputWrapperClassList}>
        <FormLabel
          ...
          required={isRequired}
          requiredColor={
            !hasError && (isBaseView || disabled) ? "grey" : undefined
          }
        />
        <div className={s["input-container"]} ref={inputRef}>
          <input
            ...
            {...rest}
            {...usePhoneMask({
              showMask: isInputTypeTel,
              element: inputRef?.current?.firstChild,
              onChange,
            })}
          />
          {/* {!excludeTypes.includes(type) && <Prefix icon={prefix} />} */}
          {!excludeTypes.includes(type as TInputType) &&
            (suffix || togglePassword || tooltip || searchType) && (
