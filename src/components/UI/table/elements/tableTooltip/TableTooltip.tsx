...
const TableTooltip = ({
  ...
  const setViewDotsContentTop = () => {
    if (tooltipRef?.current && contentRef?.current) {
      const tooltipClientRectCurrent =
        tooltipRef?.current?.getBoundingClientRect()
      const contentClientRectCurrent =
        contentRef?.current?.getBoundingClientRect()
      const contentTopShowBottomTooltip =
        tooltipClientRectCurrent?.top + tooltipClientRectCurrent?.height + 4

      if (
        contentTopShowBottomTooltip + contentClientRectCurrent?.height >
        window.innerHeight
      ) {
        return setContentTop(
          tooltipClientRectCurrent?.top - contentClientRectCurrent?.height - 4,
        )
      }
