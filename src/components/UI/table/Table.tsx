...
const Table = <T,>({
      ...
      if (columns) {
        ...
        if (hasSelect) {
          initialColumns.unshift({
            ...
            cell: ({ row }) => (
              <div
                className={s["checkbox-cell-wrapper"]}
        ...
        if (fluid && tableRef?.current) {
          ...
          const desiredColumns = shouldChangeColumnsWhenTabChanges
            ? [...defaultColumns]
            : columns
          ...
          const sumColumnsWithSize = desiredColumns
            .map((v: ColumnDef<T>) =>
              v?.size
                ? (v.size < defaultColumnMinWidth && defaultColumnMinWidth) ||
                  v?.size
                : 0,
            )
            .reduce(
              ...
              (hasSelect ? 24 : 0) +
                (hasRowFeatures ? defaultColumnMinWidth : 0),
            )
          const finallyColumnsWidth =
            (clientTableWidth - sumColumnsWithSize - 2) /
            columnsWithoutSizeProps

          initialColumns = initialColumns.map((v: ColumnDef<T>) =>
            !v?.size ? { ...v, size: finallyColumnsWidth } : v,
          )
        }
  ...
  useEffect(() => {
    const newSelectedRowsData: (T | undefined)[] = Object.keys(
      rowSelection,
    ).map(
      (key: string) =>
  ...
  useEffect(() => {
    if (
      initialSelectedRows &&
      !Object.keys(initialSelectedRows)?.length &&
  ...
  useEffect(() => {
    if (
      hasRowFeatures &&
      columns.find((v) => v.id === FEATURE_LIST_COLUMN_ID)
    ) {
      setColumns(
        columns.map((v) => {
          if (v?.id === FEATURE_LIST_COLUMN_ID)
            return featuresListColumn(rowFeatures)
          return v
        }),
      )
    }
